shinyServer(function(input, output, session) {
    ...
    output$predict <- renderTable({
      ...
      top10Katz <- top10[1:10,]
      top10Kneser <- top10[11:20,]
      colnames(top10Katz) <- c('Prediction','Method','Probability')
      x<-cbind(top10Katz,top10Kneser)
      x$Method<-NULL
      x$w<-NULL
      x$variable<-NULL
      colnames(x) <- c('Prediction',"Katz's backoff Probability","Kneser-Ney Probability")
      as.data.frame(x)
    })
    
    output$predictKatz <- renderTable({
        top10 <- prediction()[1:10,1:3]
        top10[top10$w=='BREAK',1] <- '.'
        top10 <- melt(top10)
        levels(top10$variable) <- c("Katz's backoff","Kneser-Ney")
        ...
        colnames(x) <- c('Prediction',"Katz's backoff Probability","Kneser-Ney Probability")
        print(x)
        top10Katz$Method <- NULL
        as.data.frame(top10Katz)
    })
    
    output$predictKneser <- renderTable({
      top10 <- prediction()[1:10,1:3]
      top10[top10$w=='BREAK',1] <- '.'
      top10 <- melt(top10)
      levels(top10$variable) <- c("Katz's backoff","Kneser-Ney")
      top10Katz <- top10[1:10,]
      top10Kneser <- top10[11:20,]
      colnames(top10Kneser) <- c('Prediction','Method','Probability')
      top10Kneser$Method <- NULL
      as.data.frame(top10Kneser)
    })
